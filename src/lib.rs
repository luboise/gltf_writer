...
#[deprecated(note = "Use gltf_writer::*")]
pub mod gltf;
...
type Vec3<T> = [T; 3];
#[expect(unused, reason = "Will be used later when deserializing Vec4")]
type Vec4<T> = [T; 4];
...
impl<T: std::fmt::Display> From<T> for GltfError {
...
impl Buffer {
    pub fn new(bytes: impl AsRef<[u8]>) -> Buffer {
        Buffer {
...
#[derive(Debug, Clone, Hash, PartialEq, Eq, strum::Display)]
...
pub enum VertexAttribute {
    ...
    Normal,
    Tangent,
    #[strum(to_string = "TEXCOORD_{0}")]
    TexCoord(GltfIndex),
    #[strum(to_string = "COLOR_{0}")]
    Colour(GltfIndex),
    #[strum(to_string = "JOINTS_{0}")]
    Joints(GltfIndex),
    #[strum(to_string = "WEIGHTS_{0}")]
    Weights(GltfIndex),
    #[strum(transparent)]
    User(String),
}
...
impl Accessor {
    ...
    pub fn get_bytes(&self, gltf: &Gltf) -> Result<Vec<u8>, GltfError> {
        ...
        let bv = gltf
            ...
            .ok_or(format!(
                "bufferview {} out of range [0..{}]",
                self.buffer_view_index,
                gltf.buffer_views.len()
            ))?;
        let buffer = gltf.buffers.get(bv.buffer_index as usize).ok_or(format!(
            "buffer {} out of range [0..{}]",
            bv.buffer_index,
            gltf.buffers.len()
        ))?;
        ...
        if stride < val_size {
            return Err(format!(
                "stride {stride} less than value size {val_size} for bufferview {}",
                self.buffer_view_index
            )
            .into());
        }
...
#[strum(serialize_all = "lowercase", prefix = "image/")]
#[cfg_attr(test, derive(strum::EnumIter))]
...
macro_rules! impl_type {
    ($singular:ident, $ty:ident) => {
        paste::paste! {
            impl_type!($singular, [<$singular s>], $ty);
        }
    };

    ($singular:ident, $plural:ident, $ty:ty) => {
        paste::item! {
            pub fn [<add_ $singular>](&mut self, $singular: $ty) -> GltfIndex {
                self.$plural.push($singular);
                (self.$plural.len() - 1) as GltfIndex
            }
            pub fn $plural(&self) -> &[$ty] {
                &self.$plural
            }
            pub fn [<$plural _mut>](&mut self) -> &mut Vec<$ty> {
                &mut self.$plural
            }
            pub fn $singular(&self, i: GltfIndex) -> Option<&$ty> {
                self.$plural.get(i as usize)
            }
            pub fn [<$singular _mut>](&mut self, i: GltfIndex) -> Option<&mut $ty> {
                self.$plural.get_mut(i as usize)
            }
        }
    };
}
...
impl Gltf {
    impl_type!(buffer, Buffer);
    impl_type!(animation, Animation);
    impl_type!(buffer_view, BufferView);
    impl_type!(scene, Scene);
    impl_type!(accessor, Accessor);
    impl_type!(mesh, meshes, Mesh);
    impl_type!(node, Node);
    impl_type!(material, Material);
    impl_type!(texture, Texture);
    impl_type!(image, Image);
    impl_type!(skin, Skin);

    pub fn prepare_for_export(&mut self) -> Result<(), GltfError> {
        if self.default_scene.is_none() && self.scenes.len() == 1 {
...
impl<'de> Visitor<'de> for NodeTransformVisitor {
        ...
        A: MapAccess<'de>,
    {
        ...
        while let Some(key) = map.next_key::<String>()? {
            if key == "matrix" {
                if let Some(false) = using_matrix {
